...
虽然我将程序修改过了,但是毕竟是仓促修改,不能确定0.8.3版本是否有bug
如果有bug请向我反馈,在这个帖子下面留言即可.我会注意看的!
https://tieba.baidu.com/p/6891350202?pid=151869169389
...
9. 添加了对WinLator的支持,已经实现了自动备份存档但是无法截图.
...
2. 将备份文件从cmd命令修改为C# API实现

3. 添加了对WinLator环境的支持,能够在WinLator环境下运行并自动存档,无法截图,因为键盘钩子崩溃导致无法使用快捷键

4. 添加了程序日志打印功能

5. 显示存档信息并备份/读取界面添加了右键在文件夹中打开文件的功能
";
